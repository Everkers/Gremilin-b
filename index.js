...
client.on('message' , async (msg)=>{
    if(isIncludesPrefix(msg.content)){ //check if message includes prefix (?)
        const i = msg.content.split('?')
        const command = i[1] //get only command
        if(command.match(commands.help)){ //and see if it's match our command in commands object
            msg.channel.send({embed : { //if the command is help = send this template of all our availble commands
                color: 15844367,
                author:{
            ...
            if(username == undefined){
               ...
               try{
                ...
                const upload_emoji =  await emoji.upload(base64 , res.lastMatch.champ.name , msg.guild.id) //upload emoji to the server 
                let discord_emoji = '';
                await msg.guild.emojis.forEach(  emoji =>{ if(emoji.name == res.lastMatch.champ.name.split(' ')[0]){discord_emoji =  emoji }})/*set the data to our variable*/
                 //set information to msg 
                 const info = new Discord.RichEmbed()
                 .setColor('#0099ff')
                 .setAuthor(`${res.username}`)
                 .addField('Akhir Ter7' , ` Champ : ${res.lastMatch.champ.name.toUpperCase()}${discord_emoji} \n Lane : ${res.lane} \n Time : ${res.time} \n Role : ${res.role} \n Stats : ${res.moreInfo.stats.kills}/${res.moreInfo.stats.deaths}/${res.moreInfo.stats.assists} \n Game Mode : ${res.moreInfo.mode} \n Duration : ${res.moreInfo.time} \n Natija : ${!res.moreInfo.win? 'Khasara' : 'Rb7a'}` , true)
                 ...
                 await msg.channel.send(info).then(()=>{
                   ...
                   if(err.response.status == 404){
                       msg.reply(`Mkaynch had khuna or khtna f league of legends , maybe kaynin f chi region wa7da ukhra soon gha nwli ka n supporti ga3 regions:fire:!`)
                   }
